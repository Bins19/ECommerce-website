...
?>
    <div class="row">
        ...
        <div class="col-10">
            <h4><?= $articlesFiltered[0]->getCategory() ?></h4>
            <?php
            $size = count($articlesFiltered);
            //print_r($articles);
            $i = 0;
            while($i < $size) {
                ?>
                <div class="row" style="background-color: white;">
                    ...
                    for($j = 0; $j < $gap; $j++) {
                        ?>
                        <a class="col-4 border-hover-black outer-div" href="index.php?action=article&idArticle=<?= $articlesFiltered[$i]->getIdArticle() ?>">
                            <div class="inner-div">
                                <img src="<?= $articlesFiltered[$i]->getPicture() ?>"/>
                                <h6><?= $articlesFiltered[$i]->getName() ?></h6>
                                <h6><?= $articlesFiltered[$i]->getPrice() ?> €</h6>
                            </div>
                        </a>
